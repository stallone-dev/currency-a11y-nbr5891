...
describe("Security: PII Sanitization & Data Protection", () => {
    it("deve ocultar dados sensíveis por padrão (sensitive: true)", () => {
    ...
    it("deve respeitar override pii: false no metadado do nó", () => {
        const Private = CalcAUY.create({ contextLabel: "p", salt: "s", sensitive: true });
    ...
    it("deve ocultar dados mesmo com global sensitive: false se o nó tiver pii: true", () => {
    ...
    it("deve manter tipos de operação visíveis no rastro sanitizado para auditoria estrutural", () => {
        const Private = CalcAUY.create({ contextLabel: "p", salt: "s", sensitive: true });
